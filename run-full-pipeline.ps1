...
Write-Output "[$(Get-Date -Format s)] START scrape"
npm run scrape
if ($LASTEXITCODE -ne 0) { throw "npm run scrape failed with exit code $LASTEXITCODE" }

Write-Output "[$(Get-Date -Format s)] START scrape:missing"
npm run scrape:missing
if ($LASTEXITCODE -ne 0) { throw "npm run scrape:missing failed with exit code $LASTEXITCODE" }

Write-Output "[$(Get-Date -Format s)] START prepare"
npm run prepare
if ($LASTEXITCODE -ne 0) { throw "npm run prepare failed with exit code $LASTEXITCODE" }

Write-Output "[$(Get-Date -Format s)] START export-sql"
npm run export-sql
if ($LASTEXITCODE -ne 0) { throw "npm run export-sql failed with exit code $LASTEXITCODE" }
