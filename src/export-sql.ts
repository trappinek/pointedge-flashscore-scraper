...
WHERE NOT EXISTS (SELECT 1 FROM "TipOdds" WHERE "tipId"=(SELECT "id" FROM tip_row));`;
  ...
  return `BEGIN;\n\n${statements.join("\n\n")}\n\nCOMMIT;\n\nSELECT COUNT(*) AS records,\n  COUNT(*) FILTER (WHERE t.\"status\"='won') AS won,\n  COUNT(*) FILTER (WHERE t.\"status\"='lost') AS lost,\n  COUNT(*) FILTER (WHERE t.\"status\"='refunded') AS refunded,\n  ROUND(100.0*COUNT(*) FILTER (WHERE t.\"status\"='won')/NULLIF(COUNT(*) FILTER (WHERE t.\"status\" IN ('won','lost')),0),2) AS hit_rate_percent,\n  ROUND((100.0*SUM(CASE WHEN t.\"status\"='won' THEN o.\"value\"-1 WHEN t.\"status\"='lost' THEN -1 ELSE 0 END)/NULLIF(COUNT(*) FILTER (WHERE t.\"status\" IN ('won','lost')),0))::numeric,2) AS yield_percent\nFROM \"Match\" m JOIN \"Tip\" t ON t.\"matchId\"=m.\"id\" JOIN \"TipOdds\" o ON o.\"tipId\"=t.\"id\"\nWHERE m.\"externalMatchId\" LIKE 'flashscore:%'\n  AND (t.\"reasoning\" LIKE '%[FLASHSCORE_HISTORY_BACKTEST_V1]%'\n    OR t.\"reasoning\" LIKE '%[FLASHSCORE_HISTORY_DEMO_V2]%');\n`;
}
