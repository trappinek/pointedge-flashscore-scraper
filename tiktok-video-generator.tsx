...
function buildStableTimedWords(
  ...
  // prawie cały tekst. Przy częściowych znacznikach napisy mogłyby zatrzymać
  // się wcześniej niż audio.
...
function buildNarrationScript(tip: TipOption, analysisText: string): NarrationScript {
  ...
  const introText = `Cześć. Przed nami analiza meczu ${playerA} kontra ${playerB}, przygotowana przez PointEdge.pl.`;
  const cleanAnalysis = neutralizeText(analysisText);
  const prediction = neutralizeText(replacePlayerLabels(tip.recommendedBet, tip));
  const pickText = `Na podstawie przedstawionych argumentów prognoza meczu wskazuje: ${prediction}.`;
  const ctaText = "To wszystko na dziś. Więcej tenisowych analiz znajdziesz na pointedge.pl. Do zobaczenia.";
...
function drawRibbon(ctx: CanvasRenderingContext2D, time: number): void {
  ...
  ctx.textAlign = "center";
  ctx.fillText("ANALIZA MECZU", WIDTH / 2, 138);
  ctx.restore();
}
...
function drawFooter(ctx: CanvasRenderingContext2D, progress: number): void {
  ...
  ctx.textAlign = "center";
  ctx.fillText("ANALIZA SPORTOWA  •  ATP / WTA", WIDTH / 2, 1832);
}
...
function drawIntro(ctx: CanvasRenderingContext2D, local: number, duration: number): void {
  ...
  ctx.textAlign = "center";
  ctx.fillText("ANALIZY TENISOWE ATP / WTA", 0, 190);
  ctx.restore();
}
...
function drawPick(ctx: CanvasRenderingContext2D, tip: TipOption, local: number, duration: number): void {
  ...
  ctx.fillText("PROGNOZA MECZU", WIDTH / 2, 415);
...
function drawOutro(ctx: CanvasRenderingContext2D, local: number): void {
  ...
  ctx.textAlign = "center";
  ctx.fillText("ANALIZY TENISOWE ATP / WTA", WIDTH / 2, 1000);
  ctx.restore();
...
async function convertToMp4InBrowser(
  webmBlob: Blob,
  ...
  ffmpeg.on("progress", ({ progress }) => {
    onProgress(72 + Math.round(clamp(progress) * 26));
  });
  ...
  try {
    ...
    if (exitCode !== 0) {
      throw new Error(`FFmpeg zakończył kodowanie z kodem ${exitCode}.`);
    }
    ...
    if (!bytes.byteLength) {
      throw new Error("FFmpeg utworzył pusty plik MP4.");
    }
    onProgress(99);
...
export function TikTokVideoGenerator({ tips }: Props) {
  ...
  async function generateVideo(): Promise<void> {
    ...
    try {
      ...
      setStage("convert");
      setPercent(70);
      const mp4Blob = await convertToMp4InBrowser(webmBlob, setPercent);
  ...
  const labels: Record<Stage, string> = {
    ...
    convert: "Przeglądarka koduje gotowy film MP4…",
      ...
      <div className="rounded-3xl border border-white/10 bg-zinc-950/75 p-5 sm:p-7">
        <div className="grid gap-6">
          <label className="grid gap-2">
            <span className="text-sm font-bold text-zinc-100">Wybierz analizę</span>
          ...
          <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/[0.05] p-4">
            <p className="text-sm font-black text-white">Polski lektor Microsoft Edge TTS</p>
            <p className="mt-1 text-xs leading-5 text-zinc-400">
              Naturalny głos Marek, bez płatnego konta i bez klucza API. Film jest kodowany lokalnie w tej przeglądarce — bez wysyłania pliku do magazynu Blob.
            </p>
          </div>
          ...
          <label className="grid gap-2">
            ...
            <p className="text-xs leading-5 text-zinc-500">Ten tekst jest czytany i pokazywany w filmie. Generator automatycznie dodaje powitanie, prognozę meczu i zakończenie zapraszające na pointedge.pl. Słowa kojarzące się z grami losowymi są automatycznie neutralizowane.</p>
          </label>
